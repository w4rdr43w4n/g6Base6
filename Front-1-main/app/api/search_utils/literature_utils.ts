...
export async function Import(name:string,email:string,type:string,title="",list=[],retries = 3,delay = 500) {
  try {
    ...
    if (retries > 0) {
      ...
      await sleep(delay);
      // Retry the request with one less retry attempt and double the delay
      return Import(name,email,type,title,list, retries - 1, delay);
    } else {
      // If no more retries left, throw the error
